...
export class BotService implements OnModuleInit {
  ...
  async run(): Promise<void> {
    ...
    let bids, asks, totalBid, totalAsk, bidAmount = 0, askAmount = 0, pair_id = "", buy_price, ask_price, pair_data,
        trade_last_price = 0,
        main_counter = 0,
        dynamic_increase_counter = 0, 
        dynamic_decrease_counter = 0,
        // dynamic_is_started = false, 
    ...
    const promBars = new Promise((resolve, reject) => {
      const barChecker = setInterval(async () => {
              ...
              if( trade_last_price != 0 ){ 
                if( dynamic_increase_counter >= rand_increase ) {
                  ...
                  if( trade_last_price < current_price ) {
                    ...
                    //----------------------------------------------------------------------- ADD A NEW BID WITH THE PRICE OF LOWEST ASK
                    ...
                    const addHighestBidOrderResponses = await fetch('https://limitlex.com/api' + endpoint_add_order, {
                      method: 'POST',
                      headers: {
                        'API-Key': PB_KEY_ADD_ORDER, // Your public api-key
                        'API-Sign': addHighestBidOrderSignature.toString('base64'), // Signature in base-64 format
                      },
                      body: addHighestBidUrlEncodedParams, // Parameters in body of the request
                    });
                ...
                if( dynamic_decrease_counter >= rand_decrease ) {
                  ...
                  if( trade_last_price > current_price ) { 
                    ...
                    //------------------------------------------------------------------------ ADD A NEW BID WITH THE PRIC OF LOWEST ASK
                    ...
                    // ADD A NEW ASK WITH LOWEST PRICE AND AMOUNT
                    ...
                    const addLowestAskOrderResponses = await fetch('https://limitlex.com/api' + endpoint_add_order, {
                      method: 'POST',
                      headers: {
                        'API-Key': PB_KEY_ADD_ORDER, // Your public api-key
                        'API-Sign': addLowestAskOrderSignature.toString('base64'), // Signature in base-64 format
                      },
                      body: addLowestAskUrlEncodedParams, // Parameters in body of the request
                    });
        ...
        //------------------------------------------------------------------------------------ GET  "ASK" AMOUNT BASED ON HIGHEST PRICE

        if(is_bid_order_filled == true) {

          // SEEK OUT THE ORDER_ID OF LATEST "ASK" PRICE
        ...
        if(is_ask_order_filled == true) {
          ...
          // SEEK OUT THE ORDER_ID OF LATEST "BID" PRICE
